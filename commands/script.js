module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!message.member.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR) || message.author.id != config.author.discord_id) return;
        ...
        if (scripts.some(v => v == args[0])) {
            ...
            await module.execute({ message }).catch(console.error);
        }
        else {
